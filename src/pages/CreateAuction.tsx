  ...
  useEffect(() => {
    const checkVerification = async () => {
      ...
      if (user && user.isVerified === false) {
        toast.error("Account verification pending. You cannot create auctions yet.");
        navigate("/dashboard");
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await store.createAuction({
        ...
        images: images.length > 0 ? images : ["https://picsum.photos/seed/new-auction/800/600"],
        documents: documents.length > 0 ? documents : [{ name: "Property Document.pdf", url: "#" }],
      });
                      ...
                      if (files.length > 0) {
                        try {
                          ...
                          toast.promise(Promise.all(uploadPromises), {
                            ...
                            success: (urls) => {
                              // Replace local blobs with real URLs
                              setImages(prev => {
                      ...
                      if (file) {
                        try {
                          toast.promise(store.uploadImage(file, file.name), {
                            ...
                            error: (err) => `Failed: ${err.message || 'Check bucket policies'}`
                          });
                        } catch (err) {
                          console.error(err);
                        }
